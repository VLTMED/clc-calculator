      ...
      <CardContent className="space-y-4">
          ...
          {lighting.enabled && (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: "عدد المصابيح", key: "count", step: 1, min: 0, fallback: 0 },
                  { label: "قدرة كل مصباح (W)", key: "wattEach", step: 10, min: 0, fallback: 0 },
                  { label: "ساعات التشغيل/يوم", key: "hoursPerDay", step: 0.5, min: 0, fallback: 0 },
                  { label: "معامل الحمل CLF (0-1)", key: "clf", step: 0.05, min: 0, fallback: 0 },
                  ...
                  <div key={key} className="space-y-1.5">
                    ...
                    <NumericInput className="h-8 text-xs" step={step} min={min} fallback={fallback}
                      ...
                      onChange={v => onChange({ lighting: { ...lighting, [key]: v } })} />
                  </div>
                ))}
              </div>
            ...
            <>
              {inputs.mode === "ac" && (
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium">نوع النشاط</Label>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: "عدد الأشخاص", key: "count", step: 1, fallback: 0 },
                  { label: "ساعات التواجد/يوم", key: "hoursPerDay", step: 0.5, fallback: 0 },
                  { label: "معامل الحمل CLF", key: "clf", step: 0.05, fallback: 0 },
                  ...
                  <div key={key} className="space-y-1.5">
                    ...
                    <NumericInput className="h-8 text-xs" step={step} min={0} fallback={fallback}
                      ...
                      onChange={v => onChange({ people: { ...people, [key]: v } })} />
                  </div>
                ))}
              </div>
            </>
        ...
        <div className="border rounded-lg p-3 space-y-3">
          ...
          {equipment.enabled && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "عدد الوحدات", key: "count", step: 1, fallback: 0 },
                { label: "معدل الكسب (W/وحدة)", key: "heatRate", step: 50, fallback: 0 },
                { label: "ساعات التشغيل/يوم", key: "hoursPerDay", step: 0.5, fallback: 0 },
                { label: "معامل الحمل CLF", key: "clf", step: 0.05, fallback: 0 },
              ].map(({ label, key, step, fallback }) => (
                <div key={key} className="space-y-1.5">
                  <Label className="text-xs font-medium">{label}</Label>
                  <NumericInput className="h-8 text-xs" step={step} min={0} fallback={fallback}
                    ...
                    onChange={v => onChange({ equipment: { ...equipment, [key]: v } })} />
                </div>
              ))}
            </div>
          )}
